...
if __name__ == "__main__":
    ...
    for window_length in [5,7,9]:
        length_after = int((window_length - 1)/2)
        # Data read and network data generate
        counter_hu_data = data_reader('umbc_word_freqs_top200k.txt',tail_cut, lang_selector=False)
        ...
        for num_layers in range(3,7):
            for num_hidden in [80,100,120,150,250]:
                print("Train for: num_layers:",num_layers,"num_hidden:",num_hidden,"window_length",window_length)
            
                model_dnn = model_creator_dnn(window_length, len(tag_chars), num_layers=num_layers, num_hidden=num_hidden)
    
                earlyStopping_dnn = keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, verbose=0, mode='auto')
                ...
                ev = test_ev(model_dnn,"dnn",model_params=model_params, verbose=0, hypher=pyphen.Pyphen(lang='en_EN'))
                train_param = "FFNN\t"+str(num_layers)+"\t"+str(num_hidden)+"\t"+str(window_length)+"\t"
                with open("results_FFNN_EN.txt", "a") as myfile:
                    result = train_param+'\t'+str(ev)+'\n'
                    myfile.write(result)
